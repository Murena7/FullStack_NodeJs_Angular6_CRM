...
function getOrdersMap(orders = []) {
  const daysOrders = {}
  orders.forEach((filterOrder) => {
    const date = moment(filterOrder.date).format('DD.MM.YYYY')
    ...
    daysOrders[date].push(filterOrder)
  })
  return daysOrders
}

function calculatePrice(orders = []) {
  return orders.reduce((total, filterOrder) => {
    const orderPrice = filterOrder.list.reduce((orderTotal, item) => {
